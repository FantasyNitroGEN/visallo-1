...
public class VideoFrameExtractGraphPropertyWorker extends GraphPropertyWorker {
    ...
    public void prepare(GraphPropertyWorkerPrepareData workerPrepareData) throws Exception {
        ...
        videoRotationProperty = new IntegerVisalloProperty(getOntologyRepository().getRequiredPropertyIRIByIntent("media.clockwiseRotation", PUBLIC));
        videoDurationProperty = new DoubleVisalloProperty(getOntologyRepository().getRequiredPropertyIRIByIntent("media.duration", PUBLIC));
    }
    ...
    public void execute(InputStream in, GraphPropertyWorkData data) throws Exception {
        ...
        Visibility newVisibility = new VisalloVisibility(VisalloVisibility.and(getVisibilityTranslator().toVisibilityNoSuperUser(data.getElementVisibilityJson()),VideoFrameInfo.VISIBILITY_STRING)).getVisibility();
        ...
        try {
            ...
            if (videoDuration != null && videoDuration <= ThumbnailRepository.DEFAULT_FRAMES_PER_PREVIEW){
                defaultFPSToExtract = (double)ThumbnailRepository.DEFAULT_FRAMES_PER_PREVIEW / videoRotation;
            }
            ...
            for (File frameFile : tempDir.listFiles()) {
                ...
                try (InputStream frameFileIn = new FileInputStream(frameFile)) {
                    ...
                    StreamingPropertyValue frameValue = StreamingPropertyValue.create(frameFileIn, byte[].class);
    ...
    private void saveImage(GraphPropertyWorkData data, Vertex artifactVertex, BufferedImage previewImage) throws IOException {
        ...
        StreamingPropertyValue spv = StreamingPropertyValue.create(new ByteArrayInputStream(out.toByteArray()), byte[].class);
    ...
    private BufferedImage createPreviewImage(List<Property> videoFrames, int videoRotation) throws IOException {
        ...
        if (videoRotation == 0 || videoRotation == 180) {
            previewFrameWidth = ThumbnailRepository.PREVIEW_FRAME_WIDTH;
            //previewFrameWidth = ArtifactThumbnailRepository.PREVIEW_FRAME_WIDTH;
            previewFrameHeight = ThumbnailRepository.PREVIEW_FRAME_HEIGHT;
            //previewFrameHeight = ArtifactThumbnailRepository.PREVIEW_FRAME_HEIGHT;
        } else {
            previewFrameWidth = ThumbnailRepository.PREVIEW_FRAME_HEIGHT;
            //previewFrameWidth = ArtifactThumbnailRepository.PREVIEW_FRAME_HEIGHT;
            previewFrameHeight = ThumbnailRepository.PREVIEW_FRAME_WIDTH;
    ...
    private List<Property> getFramesForPreview(Iterable<Property> videoFramesIterable) {
        ...
        double skip = (double) videoFrames.size() / (double) ThumbnailRepository.DEFAULT_FRAMES_PER_PREVIEW;
        ...
        if (results.size() < ThumbnailRepository.DEFAULT_FRAMES_PER_PREVIEW) {

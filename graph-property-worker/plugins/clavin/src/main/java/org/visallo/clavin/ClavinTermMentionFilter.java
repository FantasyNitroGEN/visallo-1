...
public class ClavinTermMentionFilter extends TermMentionFilter {
    ...
    public void prepareTargetConcepts(Configuration config) {
        ...
        //заменено getConceptsWithProperties() на getConceptsWithProperties(user.getCurrentWorkspaceId())
        for (Concept concept : ontologyRepository.getConceptsWithProperties(user.getCurrentWorkspaceId())) {
    ...
    private void prepareIris() {
        artifactHasEntityIri = ontologyRepository.getRequiredRelationshipIRIByIntent("artifactHasEntity", PUBLIC);
        stateIri = ontologyRepository.getRequiredConceptIRIByIntent("state", PUBLIC);
        countryIri = ontologyRepository.getRequiredConceptIRIByIntent("country", PUBLIC);
        cityIri = ontologyRepository.getRequiredConceptIRIByIntent("city", PUBLIC);
        geoLocationIri = ontologyRepository.getRequiredPropertyIRIByIntent("geoLocation", PUBLIC);
    }
            ...
            if (isLocation(termMention) && loc != null) {
                ...
                if (visibilityJson != null && visibilityJson.getWorkspaces().size() > 0) {
                    ...
                    for (String workspaceId : workspaceIds) {
                        workspaceRepository.updateEntityOnWorkspace(workspaceRepository.findById(workspaceId, user), id, user);

                ...
                if (graph instanceof AccumuloGraph) {
                    for (String auth : auths) {
                        ...
                        try {
                            ...
                            newAuthorizationsArray.add(auth.getBytes(String.valueOf(Constants.CLONE_PREFIX_BYTES)));
            ...
            public void run() {
                ...
                if (graph instanceof AccumuloGraph) {
                    try {
                        ...
                        byte[] authBytes = auth.getBytes(String.valueOf(Constants.CLONE_PREFIX_BYTES));
                        List<byte[]> newAuthorizationsArray = new ArrayList<>();
                        for (byte[] currentAuth : currentAuthorizations) {
    ...
    public org.vertexium.Authorizations getGraphAuthorizations(User user, String... additionalAuthorizations) {
        if (graph instanceof AccumuloGraph) {
            try {
                ...
                return (org.vertexium.Authorizations) auths;
            } catch (Exception ex) {
                throw new RuntimeException("Could not get authorizations from accumulo", ex);
            }
        } else {
